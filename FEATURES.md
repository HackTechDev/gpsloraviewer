...
- **Fenêtre « Données GPS reçues »** (`lora_monitor.py`, ouverte au démarrage de la réception, **Outils → Données GPS reçues (LoRa)…** / `Ctrl+Shift+L`) : petite fenêtre flottante non modale montrant l'âge de la dernière trame (code couleur selon la période d'émission de 10 s), l'état du fix GPS, la dernière position décodée (heure/date UTC, lat/lon, vitesse, cap, altitude/satellites/HDOP si GGA) — y compris les trames sans fix, pour vérifier la liaison radio avant d'avoir une position —, le RSSI et le numéro de paquet (lignes `#` du récepteur) avec appréciation bon/moyen/faible, les compteurs de trames (valides, avec fix, rejetées) et les 200 dernières lignes brutes ; la fermer n'arrête pas la réception
...
- **Visualisation du déplacement en temps réel** (`map_live.py`) : la trace s'allonge à chaque réception ; le marqueur **glisse** en 1 s de l'ancienne position vers la nouvelle (animation par *blitting* : seuls les éléments live sont redessinés, ~25 images/s sans rendu complet de la carte) et devient une **flèche orientée dans le sens de la marche** dès le 2ᵉ point (cap conservé si le déplacement est < 5 m, bruit GPS) ; étiquette **vitesse** (calculée entre deux réceptions) et **heure UTC** du dernier point
- **Suivi de la position** : bouton `➤ Suivre position` (barre d'outils, visible pendant LoRa Live) ; la carte se recentre quand le mobile sort des 60 % centraux de la vue ; désactivé automatiquement quand on déplace la carte à la main, recocher le bouton recentre sur la dernière position ; au premier point reçu, zoom sur la position (~1 km) si la vue est plus large que 5 km
- Graphiques et statistiques se rafraîchissent toutes les 3 s
